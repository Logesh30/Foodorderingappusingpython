...
# food_item_list contains list of food items with food id as key and their respective values as dict of details of food such as price discount stock
...
class admin:
    ...
    food_item_list ={1 :{'Name' : 'Veg roll' , 'Quantity' : '2 piece' , 'Price' : 40 , 'Discount(%)' : 10 , "Stock" : 50}, 2 :{'Name' : 'Chicken Noodles' , 'Quantity' : '200gm' , 'Price' : 150 , 'Discount(%)' : 5 , "Stock" : 55}, 3 :{'Name' : 'Chocolate Truffle Cake' , 'Quantity' : '1 piece' , 'Price' : 50 , 'Discount(%)' : 2 , "Stock" : 25}, 4 :{'Name' : 'Black Current Ice Cream' , 'Quantity' : '2 scoop' , 'Price' : 65 , 'Discount(%)' : 2 , "Stock" : 250}, 5 :{'Name' : 'Chicken Burger' , 'Quantity' : '1' , 'Price' : 120 , 'Discount(%)' : 15 , "Stock" : 150}}
    user_details = {}
    user_login =[]
    admin_details = {}
    count =5
        ...
        try:
            if self.admin_login == True:
                if self.food_id in admin.food_item_list:
                    edit_action =input \
                        ("Select from the options to edit:\nPress 1 >> Food Name\nPress 2 >> Quantity\nPress 3 >> Price\nPress 4 >> Discount\nPress 5 >> Stock \n")
                    if edit_action == '1':
...
class user(admin):
    ...
    def user_signup(self):
        ...
        if self.email in admin.user_details:
            ...
            admin.user_details[self.email].update \
                ({"Address" :self.address ,"Password" :self.password, "Previous Order": self.prev_orders})
            time.sleep(1)
            ...
            if self.user_login == True:
                ...
                if self.confirm_action == 'Y' or self.confirm_action == 'y':
                    ...
                    for i in self.order_action:
                        ...
                        admin.user_details[self.email]["Previous Order"].append \
                            ({"Name" : admin.food_item_list[i]["Name"] , "Quantity" : admin.food_item_list[i]["Quantity"] , 'Price': admin.food_item_list[i]["Price"]})
    ...
    def main_menu(self):
        print('-------------------------------MAIN MENU------------------------------------')
        self.main_action = input \
            ("Press 1 >> Place New Order \nPress 2 >> Order History \nPress 3 >> Update Profile \nPress 0 >> Log Out\n")
